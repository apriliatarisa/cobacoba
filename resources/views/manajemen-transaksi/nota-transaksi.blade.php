...
<head>
    ...
    <title>Nota transaksi - No: {{ $transaksi->id_transaksi }}</title>
    <?php
    $style = '
        <style>
            * {
...
<body onload="window.print()">
    ...
    <table width="100%" style="border: 0;">
        @foreach ($detail as $item)
            <tr>
                <td colspan="3">{{ $item->barangs->nama_barang }}</td>
            </tr>
            <tr>
                <td>{{ $item->jumlah }} x {{ ($item->barangs->harga_jual) }}</td>
                <td></td>
    ...
    <table width="100%" style="border: 0;">
        <tr>
            <td>Total Item:</td>
            <td class="text-right">{{ $transaksi->total_barang }}</td>
        </tr>
        <tr>
            <td>Diterima:</td>
            <td class="text-right">{{ ($transaksi->tunai) }}</td>
        </tr>
        <tr>
            <td>Total Harga:</td>
        ...
        <tr>
            <td>Kembali:</td>
            <td class="text-right">{{ ($transaksi->kembali) }}</td>
        </tr>
    </table>
    <hr>
